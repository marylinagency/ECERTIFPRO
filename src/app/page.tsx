              ...
              {result && (
                <>
                  ...
                  <p className="text-slate-400 mb-6">
                    حصلت على {result.score}% من {result.totalQuestions} سؤال
                  </p>
...
function PricingCard({
  ...
  return (
    <div className={`relative bg-white dark:bg-slate-800 rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl ${featured ? 'ring-2 ring-blue-500' : ''}`}>
      {featured && (
      ...
      <div className="p-6">
        <div className="flex items-center gap-2 mb-4">
          <span className="text-2xl">{levelIcons[certificate.level]}</span>
          <span className="text-sm text-slate-500 dark:text-slate-400">{certificate.level}</span>
        </div>
        ...
        <div className="space-y-2 mb-6">
          <div className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300">
            ...
            <span>{certificate.examDuration} دقيقة</span>
          </div>
          <div className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300">
            <BookOpen className="w-4 h-4 text-slate-400" />
            <span>{certificate.totalQuestions} سؤال</span>
          </div>
          <div className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300">
            <Target className="w-4 h-4 text-slate-400" />
            <span>{certificate.passingScore}% للنجاح</span>
          </div>
        </div>
        ...
        <button
          onClick={onSelect}
          className={`w-full py-3 rounded-xl font-medium text-white bg-gradient-to-r ${levelColors[certificate.level]} hover:shadow-lg transition-all`}
        >
          ابدأ الاختبار
        </button>
      </div>
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 overflow-x-hidden" dir="rtl">
      ...
      <section id="certificates" className="py-24 bg-slate-50 dark:bg-slate-900">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-full text-sm mb-4">
              <Sparkles className="w-4 h-4" />
              الشهادات المعتمدة
            </div>
            <h2 className="text-3xl sm:text-4xl font-bold text-slate-800 dark:text-white mb-4">
              شهادات مهنية معتمدة من مؤسس لغة المرجع
            </h2>
            <p className="text-xl text-slate-600 dark:text-slate-300">
              الشهادة الوحيدة المعترف بها عالمياً في لغة المرجع
            </p>
          </div>

...
const defaultSettings: SEOSettings = {
  ...
  siteKeywords: 'لغة المرجع, Al-Marjaa Language, شهادات معتمدة, برمجة عربية, ذكاء اصطناعي',
  siteUrl: 'https://ecertifpro.com',
  googleVerification: '',
  bingVerification: '',
  yandexVerification: '',
  ogTitle: 'ECERTIFPRO - المنصة الرسمية لشهادات لغة المرجع',
  ogDescription: 'احصل على شهادات معتمدة في لغة المرجع',
  ogImage: '/og-image.png',
  twitterHandle: '@almarjaa_lang',
  allowAllBots: true,
  allowGPTBot: true,
  allowClaudeBot: true,
  disallowPaths: '/admin/, /api/, /checkout/',
  sitemapEnabled: true,
};
...
export default function AdminSEOPage() {
  ...
  const [activeTab, setActiveTab] = useState<'basic' | 'verification' | 'social' | 'robots'>('basic');
          ...
          {menuItems.map((item, index) => (
            <Link
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${
                item.active ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-700 hover:text-white'
              }`}
            >
          ...
          <div className="bg-slate-800 rounded-2xl border border-slate-700 overflow-hidden">
            ...
            <div className="p-6">
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-slate-700/50 border border-slate-600 rounded-xl px-4 py-3 text-white focus:border-blue-500"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-300 mb-2">وصف الموقع (Meta Description)</label>
                    <textarea
                      value={settings.siteDescription}
                      onChange={(e) => setSettings({ ...settings, siteDescription: e.target.value })}
                      rows={3}
                      className="w-full bg-slate-700/50 border border-slate-600 rounded-xl px-4 py-3 text-white focus:border-blue-500 resize-none"
                    />
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-300 mb-2">رابط الموقع (URL)</label>
                    <input
                      type="url"
                  ...
                  <div className="bg-blue-900/20 border border-blue-500/30 rounded-xl p-4 mb-6">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-400 mt-1">سجّل موقعك في أدوات مشرفي المحركات ثم انسخ كود التحقق</p>
                      </div>
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-300 mb-2">Google Search Verification</label>
                    <div className="flex gap-2">
                      <input
                        type="text"
                        value={settings.googleVerification}
                        onChange={(e) => setSettings({ ...settings, googleVerification: e.target.value })}
                        className="flex-1 bg-slate-700/50 border border-slate-600 rounded-xl px-4 py-3 text-white"
                        placeholder="abc123..."
                        dir="ltr"
                      />
                      <a href="https://search.google.com/search-console" target="_blank" className="px-4 py-3 bg-slate-700 hover:bg-slate-600 rounded-xl">
                        <ExternalLink className="w-5 h-5" />
                      </a>
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-300 mb-2">Bing Webmaster Verification</label>
                    <div className="flex gap-2">
                      <input
                        type="text"
                        ...
                        dir="ltr"
                      />
                      <a href="https://www.bing.com/webmasters" target="_blank" className="px-4 py-3 bg-slate-700 hover:bg-slate-600 rounded-xl">
                        <ExternalLink className="w-5 h-5" />
                      </a>
                    </div>
                  </div>
              ...
              {activeTab === 'social' && (
                <div className="space-y-6">
                  <div>
                    <label className="block text-sm font-medium text-slate-300 mb-2">عنوان المشاركة (OG Title)</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-300 mb-2">وصف المشاركة (OG Description)</label>
                    <textarea
                      value={settings.ogDescription}
                      onChange={(e) => setSettings({ ...settings, ogDescription: e.target.value })}
                      rows={2}
                      className="w-full bg-slate-700/50 border border-slate-600 rounded-xl px-4 py-3 text-white resize-none"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-300 mb-2">صورة المشاركة (OG Image)</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-300 mb-2">حساب تويتر</label>
                    <input
                      type="text"
              ...
              {activeTab === 'robots' && (
                <div className="space-y-6">
                  <div className="grid md:grid-cols-2 gap-4">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-400">Google, Bing</p>
                          </div>
                        </div>
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input type="checkbox" checked={settings.allowAllBots} onChange={(e) => setSettings({ ...settings, allowAllBots: e.target.checked })} className="sr-only peer" />
                          <div className="w-11 h-6 bg-slate-600 peer-checked:bg-green-600 rounded-full after:content-[''] after:absolute after:top-[2px] after:right-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all"></div>
                        </label>
                      </div>
                    </div>
                    <div className="bg-slate-700/30 rounded-xl p-4">
                      <div className="flex items-center justify-between">
                        ...
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input type="checkbox" checked={settings.allowGPTBot} onChange={(e) => setSettings({ ...settings, allowGPTBot: e.target.checked })} className="sr-only peer" />
                          <div className="w-11 h-6 bg-slate-600 peer-checked:bg-green-600 rounded-full after:content-[''] after:absolute after:top-[2px] after:right-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all"></div>
                        </label>
                      </div>
                    </div>
                    <div className="bg-slate-700/30 rounded-xl p-4">
                      <div className="flex items-center justify-between">
                        ...
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input type="checkbox" checked={settings.allowClaudeBot} onChange={(e) => setSettings({ ...settings, allowClaudeBot: e.target.checked })} className="sr-only peer" />
                          <div className="w-11 h-6 bg-slate-600 peer-checked:bg-green-600 rounded-full after:content-[''] after:absolute after:top-[2px] after:right-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all"></div>
                        </label>
                      </div>
                    </div>
                    <div className="bg-slate-700/30 rounded-xl p-4">
                      <div className="flex items-center justify-between">
                        ...
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input type="checkbox" checked={settings.sitemapEnabled} onChange={(e) => setSettings({ ...settings, sitemapEnabled: e.target.checked })} className="sr-only peer" />
                          <div className="w-11 h-6 bg-slate-600 peer-checked:bg-green-600 rounded-full after:content-[''] after:absolute after:top-[2px] after:right-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all"></div>
                        </label>
                      </div>
                    </div>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-300 mb-2">المسارات المحظورة</label>
                    <textarea
                      value={settings.disallowPaths}
                      onChange={(e) => setSettings({ ...settings, disallowPaths: e.target.value })}
                      rows={2}
                      className="w-full bg-slate-700/50 border border-slate-600 rounded-xl px-4 py-3 text-white font-mono"
                      dir="ltr"
                    />
                  </div>
                </div>
              ...
              <div className="mt-8 pt-6 border-t border-slate-700">
                ...
                <button
                  onClick={handleSave}
                  disabled={saving}
                  className="w-full bg-gradient-to-l from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white font-bold py-4 rounded-xl transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                >
                  {saving ? (
                    <>
